/**
 * tokenExchange.ts — Exchanges an authorization code + PKCE verifier for
 * WorkOS access/refresh tokens.
 *
 * This is a public-client token exchange: no client_secret is sent.
 * PKCE's code_verifier is the sole proof of possession.
 *
 * WorkOS token endpoint (verified against docs, July 2025):
 *   POST https://api.workos.com/user_management/authenticate
 *   Content-Type: application/json
 *   Body: { grant_type, client_id, code, code_verifier }
 *
 * NOTE: WorkOS's Node SDK's `authenticateWithCode({ codeVerifier })` maps to
 * exactly this HTTP call.  We call the HTTP endpoint directly here because the
 * Node SDK is server-side only (@workos-inc/node) and must not be bundled into
 * the desktop app.  The HTTP surface is stable and documented by WorkOS.
 */
...
/** Successful response from WorkOS's authenticate endpoint. */
...
export type TokenExchangeError =
  ...
  | { readonly kind: "UNKNOWN"; readonly message: string; readonly rawStatus?: number };
...
function classifyError(status: number, body: WorkOSErrorBody): TokenExchangeError {
  const errorCode = body.error ?? body.code ?? "";
  const msg = body.error_description ?? body.message ?? "Token exchange failed.";

  if (errorCode === "invalid_grant" || errorCode === "invalid_code") {
    return { kind: "INVALID_GRANT", message: msg };
  }
  if (errorCode === "expired_code" || errorCode === "authorization_code_expired") {
...
/**
 * Exchanges an authorization `code` and `codeVerifier` for WorkOS tokens.
 *
 * @param code         - The authorization code received in the deep-link callback.
 * @param codeVerifier - The PKCE verifier generated by `generateCodeVerifier()`.
 *                       This is the FIRST and ONLY time this value leaves the app.
 * @param clientId     - The native/public WorkOS application client ID.
 *
 * @returns A typed result — never throws.  Check `.ok` before using `.data`.
 */
export async function exchangeCodeForTokens(
  ...
  try {
    ...
    return {
      ok: false,
      error: { kind: "UNKNOWN", message: "WorkOS returned a non-JSON success body." },
    };
  }
